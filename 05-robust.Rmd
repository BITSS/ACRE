...
Once you have assessed and improved the computational reproducibility of the display items for a claim, you can assess robustness by modifying some analytic choices and reporting their effects on the estimates of interest, i.e., conducting **robustness checks**. The universe of robustness checks can be very large (potentially infinite!), and they can pertain to both data analysis and data cleaning. For this reason, we distinguish between **reasonable** and **feasible** robustness checks. 

**Reasonable robustness checks** are defined as (i) sensible tests of the research question that are (ii) expected to be statistically valid and (iii) are not redundant with other specifications in the set ([Simonsohn et. al., 2018](https://urisohn.com/sohn_files/wp/wordpress/wp-content/uploads/Paper-Specification-curve-2018-11-02.pdf)). The set of **feasible robustness checks** is defined by all the specifications that can be computationally reproduced. We assume that the specifications already published in the paper are part of the reasonable set of specifications.
...
Increasing the number of feasible robustness checks requires that you, the reproducer, identify the specific line(s) in the code scripts that execute an analytical choice. An advantage of this type of contribution is that you don't need to have in-depth knowledge of the paper and its methodology to contribute. This allows you to potentially map several code files, achieving a broader understanding of the paper while building on top of others' work. The disadvantage is that you are not expected to test alternative specifications. 
...
To record a specific analytical choice in the ACRE platform, follow these steps:  
...
4. Discuss how this variation could affect the validity of the results (e.g., likely effects on omitted variable bias, measurement error, change in the Local Average Treatment Effects for the underlying population, etc.). 
