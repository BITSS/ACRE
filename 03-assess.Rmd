...
Once all possible inputs have been identified and there is a clear understanding of the connection between the outputs and inputs, you can start to assess output-specific levels of reproducibility. 
...
The following concepts will be used in this section:     
 ...
 - **Computationally Reproducible from Raw data (CRR):** The output can be reproduced with minimal effort starting from the raw data sets.
...
The assessment is made at the output level -- a paper can be highly reproducible for its main results, but suffer from low reproducibility for other outputs. The assessment is made using a 10-point scale, with 0 representing that, under current circumstances, reproducers cannot access any reproduction package, and 10 representing that all materials are accessible and the target outcome can be reproduced starting from raw data.
...
The reproducer and/or reader of this information can disagree with this subjective valuations. In this scenario, the levels should be understood as categories that can be re-ordered under different criteria. Readers of these guidelines are also encouraged to submit suggested edits using the edit button above.
...
- A master script that runs all steps    
- Version control (e.g., Git)
- Analysis using open source (statistical) software (e.g., R, Python, or Julia)
- Dynamic documentation (e.g., rMarkdown, Jupyter Notebooks, etc.)
- Computing capsule (e.g., CodeOcean, Binder, etc.)     

Congratulations! You have now completed the Assessment stage. You've provided a concrete building block of knowledge to improve understanding of the state of reproducibility in Economics. 
