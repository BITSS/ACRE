...
Finally, verify if the data sources \href{https://social-science-data-editors.github.io/guidance/addtl-data-citation-guidance.html}{are cited} in the paper and if the identified data files are available in the reproduction package. Record all this information in the SSRP using the following structure:
...
\textbf{Note:} lists of files in the data\_files and known\_missing columns should have entries separated by a semi-colon (\texttt{;}) to for the spreadsheet to be compatible with the ReproducibiliTREE.
